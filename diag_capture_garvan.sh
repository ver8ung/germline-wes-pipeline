#!/usr/bin/env bash
# Callable-region diagnosis for the Garvan/Nextera NA12878 cross-check: is the
# recall loss capture-driven (truth variants in uncovered target regions) or
# pipeline-driven (covered regions mis-/under-called or over-filtered)?
# Same logic as diag_capture.sh but for the kit-matched Nextera run:
#   - BED   = lifted Nextera Expanded Exome (the kit that produced this data)
#   - HAPPY = namespaced hap.py per-variant VCF (label=garvan_nextera)
#   - BAM   = results/bqsr/NA12878.recal.bam (mount the wes-bqsr scratch volume!)
set -euo pipefail
cd /workflow

BAM=results/bqsr/NA12878.recal.bam
HAPPY=results/benchmark/happy.garvan_nextera.vcf.gz
...
[ -f "$BAM" ] || { echo "FATAL: $BAM not found (mount wes-bqsr at /workflow/results/bqsr)"; exit 3; }
[ -f "$HAPPY" ] || { echo "FATAL: $HAPPY not found"; exit 3; }

TMP=$(mktemp -d); trap 'rm -rf "$TMP"' EXIT
bp() { awk '{s+=$3-$2} END{printf "%d", s+0}' "$1"; }

# --- eval region = Nextera targets ∩ GIAB confident (what hap.py scored over) ---
